...
* But in psychology, given that there are always similarities between people (compared to say, a block of cheese), we can also leverage knowledge from how other subjects behave -- we are probably more confident of where the red line will go now.
```{r, echo=F,out.width='80%', fig.align='center'}
...
```{r, echo=F,out.width='80%', fig.align='center'}
include_graphics('/img/g.jpg')
```



# Statistical modelling -- what are we doing?
* We want to see how a little piece of the world works -- composed of a range of related variables in some weirdly shaped multidimensional space. I labeled it 'theory space' at one point, but just a rough representation.
...
* In exchange for this, we can replace mere infinitesimal points in the large space, with links between different regions. (i.e. different values of different variables, that we have not explicitly observed) 
* These 'links' (the model!) allow for predictions about what new data would look like and how we might expect variables in a region to be related. As new data comes in, such links / predictions can be tested, and the model refined or rejected.
